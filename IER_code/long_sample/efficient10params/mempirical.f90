...
SUBROUTINE sample_mom( hatMOM, invSIG, V, nMOM, x, data_sample )

IMPLICIT NONE

INTEGER, INTENT(IN)  :: nMOM
REAL(8), INTENT(IN)  :: data_sample(size_data,num_MOM), x(num_thetta,1)
REAL(8), INTENT(OUT) :: hatMOM(nMOM,1), invSIG(nMOM,nMOM), V(nMOM,nMOM)
REAL(8)              :: ap(num_MOM)

REAL(8)  :: invSIG_dble(nMOM,nMOM), MOMtheory(nMOM,1)
INTEGER :: i, INFO, j, p, h, signout, equilibrium
...
p = (size_data - nlags)**(1.0D+0/3.0D+0)
    ...
    DO h = 1,p
!        CH = 0.0D+0
!        DO i = h+1, size_data-nlags
!            Ch = Ch + MATMUL(fz_a(:,i), TRANSPOSE(fz_a(:,i-h)))
!        END DO
!        Ch = Ch/dble(size_data - nlags)
